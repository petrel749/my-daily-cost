<template>
    <van-cell-group class="item">
        ...
        <van-cell :class="{ expense: item.pay_type == 1, income: item.pay_type == 2 }" v-for="item in bill.bills"
            :key="item.id" @click="goToDetail(item)" :title="item.type_name"
            :value="`${item.pay_type == 1 ? '-' : '+'}${item.amount}`" :label="`${$filters.transTime(item.date)}${
